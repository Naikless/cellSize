...
The \texttt{induction}-function receives the exact same input values as \texttt{cellSize}. It starts the function chain to obtain the characteristic induction time $\tau_{\mathrm{I}}$ and is further described in \autoref{induction}. The next subfunction to be called by \texttt{cellSize} is \texttt{PostShockCantera}, fully explained in \autoref{post}. It solves the Hugoniot- and Rayleigh-equations\footnote{see \citep{deto_phen}} to obtain the von Neumann conditions, present directly after the shock front.

The next step is  the calculation of the non-dimensional global activation energy $\varepsilon_{\mathrm{I}}$, which is another stability parameter required by the approximation model for the characteristic cell size and which use has been suggested by \citet{ng2,ng1}. To estimate its value, the method introduced by \citet[p.\,81]{schultz} is applied:
...
Following this, as well as in \texttt{cellSize}, the von Neumann state is provided by \texttt{PostShockCantera}: 
...
For that reason, the partial derivatives\footnote{The second one can be used to replace $c_v$, after some more rearrangements according to the Bridgman tables, see \citep{cea}} $\left(\frac{\partial \ln v}{\partial \ln p}\right)_T$ and $\left(\frac{\partial \ln v}{\partial \ln T}\right)_p$  are approximated using a central difference scheme for a step width of $\Delta p=1\cdot 10^{-8}$\;Pa and $\Delta T=1\cdot 10^{-6}$\;K respectively.
...
It makes use of the internal MATLAB\textsuperscript{\circledR} function \texttt{fsolve} to solve a functional composed of the Hugoniot- and Rayleigh-equation for the density $\rho$. 
\abk[zr]{$\rho$}{Density (kg/m$^3$)}

\lstinputlisting[firstline=27,lastline=37]{../PostShockCantera.m}
